        ...
        videos.forEach((video, index) => {
            try {
                ...
                if (video.final_score !== undefined) {
                    // Três leituras com papéis distintos, como descreve a Seção 4.6 do
                    // artigo, e é de propósito que não sejam a mesma grandeza:
                    //
                    //   POSIÇÃO  vem de final_score (Eq. 2) — responde por RISCO, e por
                    //            isso não separa Neutro de Positivo: a diferença entre
                    //            eles é de tom. Ordenar por tom rebaixa conteúdo
                    //            educativo sem que haja risco a corrigir.
                    //   COR      vem da classe predita — diz o que o item É.
                    //   NÚMERO   vem de sentiment_adequacy (Eq. 3) — gradua o quanto o
                    //            filtro considera o vídeo adequado. Informa, não ordena.
                    //
                    // Com MAIS DE UM filtro habilitado nada disso vale: o escore global
                    // da Eq. (4) é a média dos filtros e não corresponde a uma única
                    // classe, de modo que número e cor voltam os dois a sair dele.
                    const nFiltros = video.filter_scores ? Object.keys(video.filter_scores).length : 0;
                    const soSentimento = nFiltros === 1 && video.sentiment_adequacy !== undefined;

                    const exibido = soSentimento ? video.sentiment_adequacy : video.final_score;
                    const scoreValue = Math.round(exibido * 100);
                    ...
                    if (soSentimento && CORES[video.sentiment_class]) {
                        score.classList.add(CORES[video.sentiment_class]);
                    } else {
                        // Filtro de sentimento desligado, ou vários filtros ativos. A cor
                        // sai do escore, pelos limites das faixas da Eq. (2).
                        if (scoreValue >= 70) {
