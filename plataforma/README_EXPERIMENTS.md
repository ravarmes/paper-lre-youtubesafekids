...
> **Leia antes.** Este documento descreve a interface de linha de comando de `scripts/`,
> herdada da aplicação, e é genérico quanto às quatro tarefas (sentimento, toxicidade,
> linguagem, educacional). Os nomes de arquivo usados nos exemplos
> (`data/sentiment_raw.csv`, `models/sentiment/best_model.pt` e afins) são ilustrativos:
> não acompanham o repositório e precisam ser gerados antes. O corpus real é
> `../dados/corpus.csv`, e quem o localiza é `app/nlp/config.py`.
>
> **Para reproduzir os resultados do artigo, use `../experimentos/`, não estes scripts.**
> Lá o caminho é autocontido, tem ordem de execução definida e não depende de subir a
> aplicação nem de ter chave da YouTube Data API.
